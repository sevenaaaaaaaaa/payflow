...
后台入口/鉴权逻辑优化：
- 深链回跳：未登录访问子页 → 记录目标，登录后跳回（仅限本站路径，防开放重定向）
- 登录限速：连续失败达上限（默认 5 次）锁定（默认 15 分钟），登录页提示剩余
- 会话时效：空闲超时（默认 480 分钟）+ 绝对有效期（默认 7 天）
- 登录审计：success / failed / locked / logout 入事件流
- IP 识别支持 CF-Connecting-IP / X-Forwarded-For

## [1.0.5] — 2026-09-18

后台 UI/性能优化：
- 导航分组（核心 / 增长 / 系统）并支持小屏横向滚动；当前页 aria-current
- 新增 admin.css：统一页头 `.page-head`、表格滚动容器 `.table-wrap`、粘性表头、数字右对齐 `.num`、空状态 `.empty`、`.pager`、响应式表单折叠
- 列表服务端分页统一（客户/优惠券/兑换券/支付链接/推荐/佣金/提现/License/发票/Webhook/订单/审计）
- 交付页/订单页统一页头与操作区
